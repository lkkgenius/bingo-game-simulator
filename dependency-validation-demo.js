...
} = require('./utils/dependencyIntegration.js');
...
async function runDemo() {
  ...
  try {
    ...
    console.log(
      `   - 整體狀態: ${validationResults.overall.passed ? '通過' : '失敗'}`
    );
    ...
    if (validationResults.repairs.length > 0) {
      ...
      validationResults.repairs.forEach(repair => {
        console.log(
          `   - ${repair.dependency}: ${repair.success ? '成功' : '失敗'}`
        );
      });
    }
    ...
    console.log(
      `   - 可用依賴: ${healthReport.summary.availableDependencies}/${healthReport.summary.totalDependencies}`
    );
    console.log(
      `   - 關鍵依賴: ${healthReport.summary.criticalDependencies} 個`
    );
    ...
    console.log(
      `   - 檢查類別: ${Object.keys(diagnosticResults.categories).length}`
    );
      ...
      const statusIcon =
        category.status === 'healthy'
          ...
          : category.status === 'issues'
            ...
            : '❌';
      console.log(
        `   ${statusIcon} ${category.name}: ${category.score}/100 (${category.issues.length} 問題, ${category.warnings.length} 警告)`
      );
    }
    ...
    if (global.window.LineDetector) {
      ...
      console.log(
        `✅ LineDetector 回退實現測試: 檢測到 ${lines.length} 條連線`
      );
    }
    ...
    if (global.window.ProbabilityCalculator) {
      ...
      console.log(
        `✅ ProbabilityCalculator 回退實現測試: 建議位置 (${suggestion.row}, ${suggestion.col})`
      );
    }
    ...
    if (global.window.Utils) {
      ...
      const isEmpty = global.window.Utils.isCellEmpty(
        [
          [0, 1],
          [2, 0]
        ],
      ...
      console.log(
        `✅ Utils 回退實現測試: 位置驗證=${isValid}, 格子檢查=${isEmpty}`
      );
    }
    ...
    // 初始化整合管理器
    const initSuccess =
      await dependencyIntegrationManager.initializeDependencyCheck();
    ...
    console.log(
      `🕐 驗證執行時間: ${validationResults.performance.totalTime}ms`
    );
    console.log(
      `🕐 診斷執行時間: ${diagnosticResults.performance.totalTime}ms`
    );
    console.log(
      `📊 檢查的依賴項目: ${validationResults.performance.dependenciesChecked}`
    );
    console.log(
      `🧪 執行的測試: ${diagnosticResults.performance.testsExecuted}`
    );
