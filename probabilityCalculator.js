...
function initializeProbabilityCalculator() {
  ...
  if (!BaseProbabilityCalculator || !ProbConstants) {
...
if (!initializeProbabilityCalculator()) {
  // If dependencies not ready, set up a deferred initialization
  if (typeof window !== 'undefined') {
    window.addEventListener('DOMContentLoaded', () => {
      setTimeout(() => {
        if (initializeProbabilityCalculator()) {
          defineProbabilityCalculator();
        }
      }, 100);
    });
  }
} else {
  // Dependencies ready, define the class
  defineProbabilityCalculator();
}

function defineProbabilityCalculator() {
  /**
