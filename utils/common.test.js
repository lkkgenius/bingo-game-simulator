...
function testConstants() {
  ...
  if (CONSTANTS.CELL_STATES.COMPUTER !== 2) throw new Error('COMPUTER should be 2');

  // Test GAME_PHASES
  if (!CONSTANTS.GAME_PHASES) throw new Error('GAME_PHASES not defined');
  if (!CONSTANTS.GAME_PHASES.WAITING) throw new Error('WAITING phase not defined');
  if (!CONSTANTS.GAME_PHASES.PLAYER_TURN) throw new Error('PLAYER_TURN phase not defined');

  // Test LINE_TYPES
  if (!CONSTANTS.LINE_TYPES) throw new Error('LINE_TYPES not defined');
  if (!CONSTANTS.LINE_TYPES.HORIZONTAL) throw new Error('HORIZONTAL line type not defined');
  if (!CONSTANTS.LINE_TYPES.VERTICAL) throw new Error('VERTICAL line type not defined');

  // Test ALGORITHM_WEIGHTS
  if (!CONSTANTS.ALGORITHM_WEIGHTS) throw new Error('ALGORITHM_WEIGHTS not defined');
  if (!CONSTANTS.ALGORITHM_WEIGHTS.STANDARD) throw new Error('STANDARD weights not defined');
  if (!CONSTANTS.ALGORITHM_WEIGHTS.ENHANCED) throw new Error('ENHANCED weights not defined');
}
...
function testPositionValidation() {
  ...
  if (Utils.isCenterPosition(0, 0)) throw new Error('(0,0) should not be center');
  if (Utils.isCenterPosition(1, 2)) throw new Error('(1,2) should not be center');
}
...
function testBoardOperations() {
  ...
  if (board[2][2] !== CONSTANTS.CELL_STATES.EMPTY) throw new Error('Board should be empty');

  // Test board copying
  board[1][1] = CONSTANTS.CELL_STATES.PLAYER;
  const copiedBoard = Utils.copyBoard(board);
  if (copiedBoard[1][1] !== CONSTANTS.CELL_STATES.PLAYER) throw new Error('Board copy failed');

  // Modify original to test deep copy
  board[1][1] = CONSTANTS.CELL_STATES.COMPUTER;
  if (copiedBoard[1][1] !== CONSTANTS.CELL_STATES.PLAYER) throw new Error('Board copy should be deep');

  // Test board validation
  if (!Utils.isValidBoard(board)) throw new Error('Valid board should pass validation');

  const invalidBoard = [[0, 1], [2]]; // Wrong dimensions
  if (Utils.isValidBoard(invalidBoard)) throw new Error('Invalid board should fail validation');

  // Test cell checking
  if (!Utils.isCellEmpty(board, 0, 0)) throw new Error('Empty cell should be detected');
  if (Utils.isCellEmpty(board, 1, 1)) throw new Error('Occupied cell should be detected');

  // Test empty cells retrieval
  const emptyCells = Utils.getEmptyCells(board);
  if (!Array.isArray(emptyCells)) throw new Error('Empty cells should be an array');
...
function testLineOperations() {
  ...
  const horizontalCells = Utils.getLineCells(2, 2, CONSTANTS.LINE_TYPES.HORIZONTAL);
  if (horizontalCells.length !== 5) throw new Error('Horizontal line should have 5 cells');
  if (horizontalCells[0][0] !== 2) throw new Error('Horizontal line should be on row 2');
  ...
  if (verticalCells[0][1] !== 2) throw new Error('Vertical line should be on column 2');

  const diagonalCells = Utils.getLineCells(2, 2, CONSTANTS.LINE_TYPES.DIAGONAL_MAIN);
  if (diagonalCells.length !== 5) throw new Error('Diagonal line should have 5 cells');
  if (diagonalCells[2][0] !== 2 || diagonalCells[2][1] !== 2) throw new Error('Diagonal should pass through (2,2)');

  // Test line completion
  if (Utils.isLineComplete(board, horizontalCells)) throw new Error('Empty line should not be complete');
  ...
  if (!Utils.isLineComplete(board, horizontalCells)) throw new Error('Filled line should be complete');

  // Test cell counting
  if (Utils.countFilledCells(board, horizontalCells) !== 5) throw new Error('Should count 5 filled cells');
  if (Utils.countEmptyCells(board, horizontalCells) !== 0) throw new Error('Should count 0 empty cells');

  const mixedCells = Utils.getLineCells(1, 1, CONSTANTS.LINE_TYPES.HORIZONTAL);
  if (Utils.countFilledCells(board, mixedCells) !== 0) throw new Error('Should count 0 filled cells in empty line');
  if (Utils.countEmptyCells(board, mixedCells) !== 5) throw new Error('Should count 5 empty cells in empty line');
  ...
  if (edgeLines.length !== 2) throw new Error('Edge should have 2 relevant lines');
}
...
function testCacheOperations() {
  ...
  if (result1 !== 3) throw new Error('Memoized function should return correct result');
  ...
  if (callCount !== 2) throw new Error('Function should be called for new args');
...
function testUtilityFunctions() {
  ...
  if (Utils.validateInput(42, 'number') !== 42) throw new Error('Valid number should pass');
  if (Utils.validateInput('hello', 'number', 0) !== 0) throw new Error('Invalid number should return default');
  if (Utils.validateInput('hello', 'string') !== 'hello') throw new Error('Valid string should pass');
  if (Utils.validateInput(42, 'string', 'default') !== 'default') throw new Error('Invalid string should return default');
  ...
  const confidence = Utils.calculateConfidenceLevel(moves, CONSTANTS.ALGORITHM_WEIGHTS.STANDARD);
  if (confidence !== 'very-high') throw new Error('Large difference should give very-high confidence');
  ...
  if (debounceCount !== 0) throw new Error('Debounced function should not execute immediately');
  ...
  if (throttleCount !== 1) throw new Error('Throttled function should execute once immediately');

  // Test format duration
  if (Utils.formatDuration(500) !== '500.00ms') throw new Error('Milliseconds should format correctly');
  if (Utils.formatDuration(1500) !== '1.50s') throw new Error('Seconds should format correctly');
...
function testErrorHandling() {
  ...
  if (error.name !== 'GameError') throw new Error('Error name should be GameError');
  if (error.message !== 'Test error') throw new Error('Error message should be preserved');
  if (error.type !== 'test-type') throw new Error('Error type should be preserved');

  // Test safe JSON operations
  const validJson = '{"test": "value"}';
  const parsed = Utils.safeJsonParse(validJson);
  if (parsed.test !== 'value') throw new Error('Valid JSON should parse correctly');

  const invalidJson = '{"invalid": json}';
  const defaultParsed = Utils.safeJsonParse(invalidJson, { default: true });
  if (!defaultParsed.default) throw new Error('Invalid JSON should return default');

  const obj = { test: 'value' };
  const stringified = Utils.safeJsonStringify(obj);
  if (stringified !== '{"test":"value"}') throw new Error('Object should stringify correctly');

  // Test circular reference handling
  const circular = { a: 1 };
  circular.self = circular;
  const circularStringified = Utils.safeJsonStringify(circular, '{}');
  if (circularStringified !== '{}') throw new Error('Circular reference should return default');
}
...
function testPerformanceHelpers() {
  ...
  if (Utils.formatDuration(0) !== '0.00ms') throw new Error('Zero duration should format correctly');
  if (Utils.formatDuration(999) !== '999.00ms') throw new Error('Sub-second should format as ms');
  if (Utils.formatDuration(1000) !== '1.00s') throw new Error('One second should format as s');
  if (Utils.formatDuration(65000) !== '1m 5.00s') throw new Error('Minutes should format correctly');
